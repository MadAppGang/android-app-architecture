...
import com.madappgang.recordings.models.*
...
internal class RecorderActivity :
    ...
    override fun onDialogPositiveClick(requestId: String, value: String) {
        if (requestId == saveTrackRequestId) {
            saveTrackJob?.cancel()
            saveTrackJob = saveTrack(track, value, folder)
        }
    }
    ...
    private fun saveTrack(track: Track, newName: String, destination: Folder) = launch(uiContext) {
        progressBar.makeVisible()

        val result = async(bgContext) { track.save(destination, newName) }.await()
